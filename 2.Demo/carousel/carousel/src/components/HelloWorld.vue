  ...
  <div id="slider">
    <div class="window" @mouseover="stop" @mouseleave="play">
      <ul class="container" :style="containerStyle">
        ...
        <li v-for="(item,index) of sliders" :key="index">
      ...
      <ul class="dots">
        <li v-for="(dot,index) of sliders" 
        :class="{dotted: index+1 === currentIndex}" 
        :key="index" @click="jump(index+1)"></li>
      </ul>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      animate(des,dir,speed) {
        if(this.temp) {
          window.clearInterval(this.temp)
          this.temp = null
        }
        this.temp = window.setInterval( ()=> {
          if((dir === -1 && des < this.distance) || (dir === 1 && des > this.distance)){
            this.distance += speed * dir
          }else{
      ...
      jump(index) {
        const dir = index - this.currentIndex >= 0 ? -1 : 1
        const offset = Math.abs(index - this.currentIndex) * 600
        const jumpSpeed = Math.abs(index - this.currentIndex) === 0 ? 
        this.speed : Math.abs(index - this.currentIndex) * this.speed
        this.move(offset,dir,jumpSpeed)
      },
      play() {
        if(this.timer) {
